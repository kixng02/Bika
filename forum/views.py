...
def delete_post(request, post_id):
    try:
        # Ensure the post exists and is associated with the current user or has the necessary permissions
        post = Posts.objects.get(pk=post_id)
        
        # Check if the user has the necessary permissions to delete the post
        if post.user != request.user:
            return JsonResponse({'success': False, 'error': 'You do not have permission to delete this post.'})
